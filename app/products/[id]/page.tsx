...
export default function ProductReviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="rounded-lg border border-neutral-200 bg-white p-5">
        ...
        {product.shortDescription && (
          <div className="space-y-4">
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-neutral-700">Long description</label>
              <textarea
                className="w-full rounded-md border border-neutral-300 px-3 py-2 text-sm"
                rows={6}
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-neutral-700">Attributes</label>
              <div className="space-y-2">
                ...
                {product.attributes.length === 0 && (
                  <p className="text-sm text-neutral-400">No attributes were found in the source text.</p>
                )}
              </div>
            </div>
            <button
            ...
            {product.images.map((img, i) => (
              <button
                ...
                className={`overflow-hidden rounded-lg border-2 text-left ${
                  img.selected ? "border-neutral-900" : "border-transparent"
                }`}
              >
      ...
      <section className="flex items-center gap-3 rounded-lg border border-neutral-200 bg-white p-5">
        <button
        ...
        {product.errorMessage && (
          <span className="text-sm text-red-600">{product.errorMessage}</span>
        )}
      </section>
    </div>
  );
